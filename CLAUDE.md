...
### Question data shape
Field names are full words in snake_case, not abbreviated. Each entry in a `src/data/<topicId>.json` array looks like:
```json
{"id":1,"text":"...","has_image":true,"image":"data:image/webp;base64,...","explanation":"...","options":[{"text":"...","correct":false}, ...]}
```
There is no letter (`а`/`б`/`в`/`г`...) anywhere in the data or in code — an option is identified purely by its position in the `options` array. Every page displays that 1-based position as the badge/label (`options.forEach(function(opt, idx){...(idx+1)...})`), and "which option did I pick" is tracked as that same 0-based array index, never a letter — this reads more consistently anyway, since some official explanations already refer to options by number ("у пунктах 1 та 2") rather than by letter. Images are embedded directly as base64 data URIs inside the JSON — there is no runtime reference to the `photos/` folder from the HTML or JSON.
...
Every localStorage value is JSON with full snake_case field names (`question_index`, not `qi`; `correct_streak`, not `s`) — never truncated. Anywhere a topic needs identifying inside a value that's shared across topics (the mistakes store, the exam's recent-question history), it's stored as a small 1-based **number** — `topic_id` — instead of repeating a string like `"16_2"`; a fixed 41-entry `TOPIC_ORDER` array (duplicated as a small internal constant in `mistakes-store.js` and `exam-engine.js`, since neither loads `topics-data.js`) maps between that number and the real topic id string. The public JS-facing functions on `PDRMistakes` still take/return the topic id as its usual string (e.g. `PDRMistakes.recordWrong(topicId, qid)`) — only the serialized JSON itself uses the numeric form — so nothing calling into the store had to change. Per-topic key *names* (`pdr_t<ID>_best_v1`) still use the topic's own string id, same as `PDR_TOPIC_ID`; only values shared/repeated across topics were worth compacting.

This schema replaced an earlier one (camelCase field names, `topicId` stored as a string, `answers[i]` as a `{letter, correct}` object). Every load function that reads one of these keys (`loadBest`/`loadProgress` in `quiz-engine.js`, `load` in `mistakes-store.js`, `loadLastResult`/`getRecentHistory` in `exam-engine.js`) detects the old shape on read (e.g. a `date` field instead of `timestamp`, or an `orderIds`/`topicId` camelCase property) and transparently rewrites it to the new shape *in place*, once, so real progress/best-scores/mistakes/exam-history a user already has saved in their browser survive a deploy of this schema rather than silently reading as empty. An old saved answer's letter (`а`/`б`/`в`/`г`/`д`) converts losslessly to an index (`0`/`1`/`2`/`3`/`4`), since letters always matched an option's position in the original data.

For a given page's `PDR_TOPIC_ID` (e.g. `01`, `08_1`, `16_2`):
- `pdr_t<ID>_best_v1` — best score `{score, total, timestamp}`
- `pdr_t<ID>_progress_v1` — in-progress attempt `{order_ids, answers, question_index, timestamp}`, written on every answer/next/back, cleared on finish; lets a quiz resume after closing the tab (subject to the `file://`/Firefox caveat above). `answers[i]` is the 0-based index of the option chosen for that question, or `null` if unanswered — correctness is always derived from `question.options[answers[i]].correct` at read time, never stored redundantly (and note it's a real array index, so `0` is a valid answer — every check in the code is `!== null`, not a truthiness check, since `0` is falsy).

### Cross-topic mistakes store (`src/scripts/mistakes-store.js`) + blitz page (`mistakes.html`)
Every wrong answer, on any of the 41 quiz pages, is recorded into one shared localStorage key `pdr_mistakes_v1` — a JSON array of lean entries `{topic_id, question_id, correct_streak}` (topic_id numeric, per the convention above), nothing else. In particular there's no embedded question snapshot: whatever needs to actually render a mistake's text/options/explanation (currently just `mistakes.html`) fetches it from `src/data/<topicId>.json` via `PDRTopicLoader.loadTopic` and looks the question up by id — the store itself stays a tiny, disposable index, not a second copy of the question bank. `quiz-engine.js`'s `selectOption` calls `PDRMistakes.recordWrong(topicId, qid)` on every wrong answer (never on a normal quiz's correct answer).

`mistakes.html` (root-level, own JS at `src/scripts/mistakes-blitz.js`, reuses `quiz.css`) is the "робота над помилками" blitz: pick "all" or specific topics via checkboxes (counts come straight from `PDRMistakes.byTopic()`), then on "Почати бліц" it loads (with a small "Завантаження питань…" state) every topic id referenced by the selected mistakes via the shared loader, maps each `{topicId, qid}` entry to its actual question, and runs a quiz over that in-memory list — dropping (and pruning from the store via `removeEntry`) any entry whose question no longer exists in the bank. Answering a pooled question correctly calls `PDRMistakes.recordCorrect(topicId, qid)`, which increments that entry's `correctStreak`; reaching `MASTERY_STREAK` (4, i.e. more than 3 correct in a row) deletes the entry from the pool entirely. A wrong answer anywhere (including inside the blitz itself) calls `recordWrong` again, which resets `correctStreak` back to 0 — so mastery requires 4 *consecutive* correct answers, and one slip-up starts that streak over. Only the blitz page ever calls `recordCorrect`; normal topic quizzes only ever add to the pool, never remove from it. The in-flight blitz's own "how many correct in a row" note (`masteryNoteFor`) is never stashed per-answer either — it re-reads `PDRMistakes.get(topicId, qid)` live, which stays correct on revisiting an answered question since each question is only ever answered once per blitz session.
...
The exam itself is deliberately blind, unlike every other quiz on the site: selecting an option just highlights it (`.opt.selected`, no correct/wrong reveal, no explanation), answers stay changeable (`answers[i]` is again a 0-based option index or `null`, exactly like the quiz engine's), and a question-number grid (`#qgrid`) lets you jump anywhere — all matching real exam UX. Nothing is scored until you finish (early via "Завершити достроково", automatically when the 20-minute countdown hits 0, or after question 20), at which point `MAX_WRONG_TO_PASS` (2) decides pass/fail, an unanswered question counts as wrong, every wrong question is fed into the same `PDRMistakes.recordWrong` store the rest of the site uses, and a full per-question review (reusing the `.litem` list-mode styles from `quiz.css`) is rendered. The last result persists to `pdr_exam_last_v1` as `{score, total, wrong, passed, timestamp, timed_out}` (read by `index.html`'s `#exam-last-pill`), and the just-used questions are appended as `{topic_id, question_id}` pairs to a capped rolling history at `pdr_exam_recent_v1` (`RECENT_EXAMS_TO_AVOID` entries) that later exam builds try to exclude first, falling back to the full pool if excluding would leave too few questions — a lightweight stand-in for the fuller spaced-repetition weighting described as optional/future work.
